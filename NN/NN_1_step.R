...
# входная матрица (input matrix)
X <- matrix(c(1,0,1,0,1,0,1,1,0,1,0,1),nrow = 3, ncol=4,byrow = TRUE)
X
# выходная матрица (output matrix)
Y <- matrix(c(1,1,0),byrow=FALSE)
Y
# функция сигмоиды (sigmoid function)
sigmoid <- function(x){
    1/(1+exp(-x))
}

# производная сигмоиды (derivative of sigmoid function)
derivatives_sigmoid<-function(x){
    x*(1-x)
}

# установка переменных (variable initialization)
epoch <- 5000
lr <- 0.1
...
# задаем веса и смещения (weight and bias initialization)
...
# Результат после обучения (result)
output
Y
# Оценка ошибки (MSE)
